...
public class EncodingMain2 {
    ...
    private static void test(String text, Charset encodingCharset, Charset decodingChartset) {
        byte[] encoded = text.getBytes(encodingCharset);
        String decoded = new String(encoded, decodingChartset);
        System.out.printf("%s -> [%s] 인코딩 -> %s %sbyte -> [%s] 디코딩 -> %s\n",
                text, encodingCharset, Arrays.toString(encoded), encoded.length, decodingChartset, decoded
        );
